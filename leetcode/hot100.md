...
```
i指向不为0的位置。
- 在初始阶段，`i`和`j`同步移动，`nums[i]`可能不是 0
- 当第一次遇到 0 时，`j`会继续前进，而`i`会停留在 0 的位置
# 11 盛最多水的容器
#双指针
```java
 while(i<=j){
...
```java
    public int lengthOfLongestSubstring(String s) {
        ...
        int rk = -1, ans = 0;
        for (int i = 0; i < n; i++) {
            if (i != 0)//这是在窗口左端右移时，把前一个字符从窗口中移除。
...
- 集合用于存储当前窗口中已经包含的字符，左指针移动的时候，从集合中移除一个元素
...
public int subarraySum(int[] nums, int k) {
	int ans=0,s=0;
	Map<Integer,Integer> cnt=new HashMap<>(nums.length);//记录前缀和出现的次数
	for(int x:nums){
...
- [n] deque里面存储的是递减的数列，但是数列的个数不定，并不是题目规定的那个值，二者之间没有关系
...
```
1.由于缺失的最小正数一定在[1,len+1],故要通过交换元素调整数组中索引和值对应的状态i，即`nums[i]==i+1`
交换的条件是：
- nums[i]为正数
- nums[i]不超过数组的长度
- 我们想把数字 `1~n` 放到下标 `0~n-1` 上，比如：
- 数字 `1` 放在 `index 0`
- 数字 `2` 放在 `index 1`
- ...
- 数字 `n` 放在 `index n-1`
这样我们就可以通过**数组位置判断**哪个数缺失了。

2.找出第一个不符合`nums[i]==i+1`的索引i，i+1就是缺失的最小正数
3.若数组已符合要求，则返回len+1
...
- 统计t中每次字符的频次
- [2] 二、过滤s中无关字符，加速滑窗匹配
- 把 `s` 中**不是 `t` 里的字符全部过滤掉**；
- 只保留 `t` 关心的字符和它们的原始位置；
- [3] 三、主逻辑
- 窗口右边不断扩展，直到包含 `t` 所有字符；
- 窗口满足后，尝试移动左边界缩小窗口（必要字符不代表每次出现都是“必须的”——只有当它的 **次数降到低于需求时** 才影响 `formed` 的值）
- 每次窗口合法就尝试记录当前最小的长度和位置。
# 73 矩阵置0
#哈希 #矩阵
```java
for (int i = 0; i < m; i++) {
...
```
第一次遍历：
- 如果 `matrix[i][j] == 0`，就把它对应的行头`matrix[i][0]`和列头`matrix[0][j]`置为0，作为**标记**。
- 特殊处理：**第一列**不能混用，因为 `matrix[0][0]` 既属于第一行也属于第一列，会冲突，所以我们用一个单独变量 `flagCol0` 来记录**第一列是否需要置0**。
第二次遍历：
- 从下往上遍历是为了避免提前把第一行或第一列改了导致影响标记判断
# 54 螺旋矩阵
#矩阵
```java
...
class Solution {
    public double findMedianSortedArrays(int[] nums1, int[] nums2) {
        ...
        while (left <= right) {
            int cut1 = (left + right) / 2;           // nums1 的分割位置
            int cut2 = (m + n + 1) / 2 - cut1;       // nums2 的分割位置

            // 边界处理，若 cut1==0，说明左边没有元素
...
- nums1 左半部分最大值 <= nums2 右半部分最小值，且 nums2 左半部分最大值 <= nums1 右半部分最小值. 这个条件意味着：**两个数组的左半部分合并后，所有元素都小于等于右半部分合并后的所有元素**，这是计算中位数的前提
...
```
- prev为已处理的反转部分的头部，cur当前处理节点，next记录cur.next
要交换的对象只有pre和cur指针，next指针只是一个锚点。
在考虑这道题的时候，不要像之前一样考虑头插的过程，只需要考虑把箭头反转就可以。

```java
//合并链表，创建新节点val为-1，再用另一个指针进行操作。
...
```
- `sum % 10` 是当前位的数字
- 第一次需要初始化链表（`head == null`），之后都通过 `tail.next` 添加新节点，并更新 `tail`
...
```java
    ...
    public ListNode swapPairs(ListNode head) {
        ...
        ListNode node3 = node2.next;//下一组的起点
		//递归地对后续链表 `node3` 开始的部分进行两两交换，结果接到 `node1` 的后面。
